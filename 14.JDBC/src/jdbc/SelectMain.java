...
public class SelectMain {
	...
	public static void main(String[] args) {
		...
		// 3. 쿼리문(sql) 선언
			...
			//쿼리문실행-sqldeveloper에서의 ctrl+enter
			java.sql.Statement stmt = conn.createStatement(); //쿼리실행의 기능을 갖고 있다.
			ResultSet rs = stmt.executeQuery(sql);
			System.out.println("조회실행");
			System.out.println("사번 \t 성명 \t 부서코드 \t 업무코드 \t 입사일자");
			while(rs.next()) { //커서이동하기
